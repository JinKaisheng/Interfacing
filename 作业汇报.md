...
插件产品版本由根 CMake 的同名变量定义，通过 `target_compile_definitions` 提供
给对应插件，并用同一个变量生成 YAML 的 `class.ver`，两处不再各写一份字符串。
A/B 默认均为 `1.0.0`，但可各自升级；Interface 契约仍由 `INTERFACE_VERSION` 定义。
这里的编译宏等价于 `#define INTERFACING_IMPL_A_PLUGIN_VERSION "1.0.0"`，只是
定义由构建系统传给编译器，而不是手写在 `.cpp` 中。宏仍然展开成字符串。
...
A/B 的接口契约、插件产品和 CMake 项目发布版本默认都是 `1.0.0`，但已经分别定义，
不能因为数值相同而认为是同一个版本。Legacy 测试插件使用
`INTERFACING_LEGACY_PLUGIN_VERSION`（测试 CMake 中为 `0.9.0`）生成 HCL/YAML；
`GetVersion()` 仍按原代码返回 `"0.9.0"`，不改接口语义。因此仍然是“第一层通过、
第二层拒绝”；不把 Legacy 的插件版本宏改成当前主程序的接口宏。
...
2. **更多异常测试。** 补充缺失 `class.ver`、错误动态类名、`.so` 不存在、非法
   动态库、缺少工厂符号和工厂返回空指针。
3. **独立发布回归。** 在已分开的版本定义基础上，补充不同发布批次插件与主程序的
   兼容组合测试及 ABI 检查；版本字符串校验本身不等于二进制兼容性证明。
